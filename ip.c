...
int ip_iface_register(struct net_device *dev, struct ip_iface *iface){
    ...
    // 17-1: インタフェース登録時にそのネットワーク宛の経路情報を自動で登録する
...
static ssize_t ip_output_core(struct ip_iface *iface, uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, ip_addr_t nexthop, uint16_t id, uint16_t offset)
{
    ...
    //
    debugf("dev=%s, dst=%s, protocol=%u, len=%u, nexthop=%s",
        NET_IFACE(iface)->dev->name, ip_addr_ntop(dst, addr, sizeof(addr)), protocol, total, ip_addr_ntop(nexthop, addr, sizeof(addr)));
        ip_dump(buf, total);
